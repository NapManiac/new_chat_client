...
public class UserInfoMessage extends Packet {
    ...
    public byte[] encode() throws UnsupportedEncodingException {
        ...
        System.arraycopy(Util.int2bytes(Util.INFO_SEARCH), 0, buffer, pos, 4);
    ...
    public void process() {
        //接受到信息并显示
        Log.d("UserInfo", "进入User");
        Message message=new Message();
        Bundle bundle = new Bundle();
        if (where == Util.INFO_SEARCH) {
            message.what = AddFriendActivity.SHOW_INFO;
            bundle.putString("id", getReceiveUser());
            bundle.putString("name", name);
            bundle.putString("motto", motto);
            message.setData(bundle);
            AddFriendActivity.addFriendActivity.getHandler().sendMessage(message);
        } else if (where == Util.INFO_SEE_FRIEND) {

        } else if (where == Util.INFO_SEE_NEW_FRIEND) {

        } else if (where == Util.INFO_MAIL_INIT) {
            message.what = MainActivity.INIT_MAIL;
            bundle.putSerializable("contacts", new Contacts(getReceiveUser(), name, motto));
            message.setData(bundle);
            MainActivity.mainActivity.getMsghandler().sendMessage(message);

        } else if (where == Util.INFO_REQUESTL_INIT) {
            message.what = MainActivity.INIT_REQUEST;
            bundle.putSerializable("contacts", new Contacts(getReceiveUser(), name, motto));
            message.setData(bundle);
            MainActivity.mainActivity.getMsghandler().sendMessage(message);

        } else if (where == Util.INFO_REQUESTL_ADD) {
            ...
            Log.d("request_contacts", getReceiveUser() + " aaaa");
